...
const ProductDetailsImage = ({ product }: { product: ProductType }) => {
  const [thumbnail, setThunbnail] = useState(product?.thumbnail);
  ...
  return (
    <div>
      ...
      <div className="flex gap-1">
        <div className="w-24 h-24 border hover:border-primary2" onClick={() => setThunbnail(product.thumbnail)}>
          <img className="w-full h-full object-cover" src={product.thumbnail} alt={product.title} />
        </div>
        {product.images.map((img) => (
          <div key={img} className="w-24 h-24 border hover:border-primary2" onClick={() => setThunbnail(img)}>
